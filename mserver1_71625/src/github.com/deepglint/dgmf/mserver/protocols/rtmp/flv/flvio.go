...
func (this Tag) ChannelLayout() av.ChannelLayout {
	if this.SoundType == SOUND_MONO {
		return av.CH_MONO
	} else {
		return av.CH_STEREO
	}
...
func (this *Tag) audioParseHeader(b []byte) (n int, err error) {
	if len(b) < n+1 {
	...
	flags := b[n]
	n++
	this.SoundFormat = flags >> 4
	this.SoundRate = (flags >> 2) & 0x3
	this.SoundSize = (flags >> 1) & 0x1
	this.SoundType = flags & 0x1
...
func (this Tag) audioFillHeader(b []byte) (n int) {
	var flags uint8
	flags |= this.SoundFormat << 4
	flags |= this.SoundRate << 2
	flags |= this.SoundSize << 1
	flags |= this.SoundType
	b[n] = flags
	n++

	switch this.SoundFormat {
	case SOUND_AAC:
		b[n] = this.AACPacketType
		n++
	}
...
func (this *Tag) videoParseHeader(b []byte) (n int, err error) {
	if len(b) < n+1 {
		err = fmt.Errorf("videodata: parse invalid")
		return
	}
	flags := b[n]
	this.FrameType = flags >> 4
	this.CodecID = flags & 0xf
	n++

	if this.FrameType == FRAME_INTER || this.FrameType == FRAME_KEY {
		if len(b) < n+4 {
			err = fmt.Errorf("videodata: parse invalid")
			return
		}
		this.AVCPacketType = b[n]
		n++

		this.CompositionTime = pio.I24BE(b[n:])
		n += 3
	}
...
func (this Tag) videoFillHeader(b []byte) (n int) {
	flags := this.FrameType<<4 | this.CodecID
	b[n] = flags
	n++
	b[n] = this.AVCPacketType
	n++
	pio.PutI24BE(b[n:], this.CompositionTime)
	n += 3
	return
}

func (this Tag) FillHeader(b []byte) (n int) {
	switch this.Type {
	case TAG_AUDIO:
		return this.audioFillHeader(b)

	case TAG_VIDEO:
		return this.videoFillHeader(b)
	}

	return
}

func (this *Tag) ParseHeader(b []byte) (n int, err error) {
	switch this.Type {
	case TAG_AUDIO:
		return this.audioParseHeader(b)

	case TAG_VIDEO:
		return this.videoParseHeader(b)
	}
