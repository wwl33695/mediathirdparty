...
Instantaneous Decoder Refresh (IDR). This VCL NALU is a this contained image slice. That is, an IDR can be decoded and displayed without referencing any other NALU save SPS and PPS.
...
func ParseSPS(data []byte) (this SPSInfo, err error) {
	...
	if this.ProfileIdc, err = r.ReadBits(8); err != nil {
		return
	}
	...
	// level_idc
	if this.LevelIdc, err = r.ReadBits(8); err != nil {
		return
	}
	...
	if this.ProfileIdc == 100 || this.ProfileIdc == 110 ||
		this.ProfileIdc == 122 || this.ProfileIdc == 244 ||
		this.ProfileIdc == 44 || this.ProfileIdc == 83 ||
		this.ProfileIdc == 86 || this.ProfileIdc == 118 {
	...
	if this.MbWidth, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	this.MbWidth++

	if this.MbHeight, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	this.MbHeight++
	...
	if frame_cropping_flag != 0 {
		if this.CropLeft, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if this.CropRight, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if this.CropTop, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if this.CropBottom, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
	}

	this.Width = (this.MbWidth * 16) - this.CropLeft*2 - this.CropRight*2
	this.Height = ((2 - frame_mbs_only_flag) * this.MbHeight * 16) - this.CropTop*2 - this.CropBottom*2
...
func (this CodecData) AVCDecoderConfRecordBytes() []byte {
	return this.Record
}

func (this CodecData) SPS() []byte {
	return this.RecordInfo.SPS[0]
}

func (this CodecData) PPS() []byte {
	return this.RecordInfo.PPS[0]
}

func (this CodecData) Width() int {
	return int(this.SPSInfo.Width)
}

func (this CodecData) Height() int {
	return int(this.SPSInfo.Height)
}

func NewCodecDataFromAVCDecoderConfRecord(record []byte) (this CodecData, err error) {
	this.Record = record
	if _, err = (&this.RecordInfo).Unmarshal(record); err != nil {
		return
	}
	if len(this.RecordInfo.SPS) == 0 {
		err = fmt.Errorf("h264parser: no SPS found in AVCDecoderConfRecord")
		return
	}
	if len(this.RecordInfo.PPS) == 0 {
		err = fmt.Errorf("h264parser: no PPS found in AVCDecoderConfRecord")
		return
	}
	if this.SPSInfo, err = ParseSPS(this.RecordInfo.SPS[0]); err != nil {
...
func NewCodecDataFromSPSAndPPS(sps, pps []byte) (this CodecData, err error) {
	recordinfo := AVCDecoderConfRecord{}
	...
	this.Record = buf
	if this.SPSInfo, err = ParseSPS(sps); err != nil {
		return
	}
	return
}
...
func (this *AVCDecoderConfRecord) Unmarshal(b []byte) (n int, err error) {
	if len(b) < 7 {
		err = ErrDecconfInvalid
		return
	}

	this.AVCProfileIndication = b[1]
	this.ProfileCompatibility = b[2]
	this.AVCLevelIndication = b[3]
	this.LengthSizeMinusOne = b[4] & 0x03
	...
	for i := 0; i < spscount; i++ {
		...
		this.SPS = append(this.SPS, b[n:n+spslen])
		n += spslen
	}
	...
	for i := 0; i < ppscount; i++ {
		...
		this.PPS = append(this.PPS, b[n:n+ppslen])
		n += ppslen
	}
...
func (this AVCDecoderConfRecord) Len() (n int) {
	n = 7
	for _, sps := range this.SPS {
		n += 2 + len(sps)
	}
	for _, pps := range this.PPS {
		n += 2 + len(pps)
	}
	return
}

func (this AVCDecoderConfRecord) Marshal(b []byte) (n int) {
	b[0] = 1
	b[1] = this.AVCProfileIndication
	b[2] = this.ProfileCompatibility
	b[3] = this.AVCLevelIndication
	b[4] = this.LengthSizeMinusOne | 0xfc
	b[5] = uint8(len(this.SPS)) | 0xe0
	n += 6
...
func (this SliceType) String() string {
	switch this {
	case SLICE_P:
		return "P"
	case SLICE_B:
