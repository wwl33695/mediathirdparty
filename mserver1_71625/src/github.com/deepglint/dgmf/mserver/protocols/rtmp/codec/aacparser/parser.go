...
func (this MPEG4AudioConfig) IsValid() bool {
	return this.ObjectType > 0
}

func (this *MPEG4AudioConfig) Complete() {
	if int(this.SampleRateIndex) < len(sampleRateTable) {
		this.SampleRate = sampleRateTable[this.SampleRateIndex]
	}
	if int(this.ChannelConfig) < len(chanConfigTable) {
		this.ChannelLayout = chanConfigTable[this.ChannelConfig]
	}
	return
}
...
func (this CodecData) MPEG4AudioConfigBytes() []byte {
	return this.ConfigBytes
}

func (this CodecData) ChannelLayout() av.ChannelLayout {
	return this.Config.ChannelLayout
}
...
func (this CodecData) PacketDuration(data []byte) (dur time.Duration, err error) {
	dur = time.Duration(1024) * time.Second / time.Duration(this.Config.SampleRate)
	return
}

func NewCodecDataFromMPEG4AudioConfig(config MPEG4AudioConfig) (this CodecData, err error) {
	b := &bytes.Buffer{}
	WriteMPEG4AudioConfig(b, config)
	return NewCodecDataFromMPEG4AudioConfigBytes(b.Bytes())
}

func NewCodecDataFromMPEG4AudioConfigBytes(config []byte) (this CodecData, err error) {
	this.ConfigBytes = config
	if this.Config, err = ParseMPEG4AudioConfigBytes(config); err != nil {
