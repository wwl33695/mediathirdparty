...
// Parse a string representation of a Subject header into a slice of at most one Subject header object.
func parseSubject(headerName string, headerText string) (
	headers []base.SipHeader, err error) {
	s := strings.Split(headerText, ",")

	if len(s) != 2 {
		err = errors.New("parser subject header err!")
		return
	}
	...
	rs := strings.Split(s[0], ":")
	if len(rs) != 2 {
		err = errors.New("parser subject header err!")
		return
	}

	subjectHeader := base.Subject{ss[0], ss[1], rs[0], rs[1]}
	headers = []base.SipHeader{&subjectHeader}
	return
