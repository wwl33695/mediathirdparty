...
import (
	...
	"github.com/deepglint/dgmf/mserver/protocols/sip/utils"
)
...
func (p *params) Copy() Params {
	...
	for _, k := range p.Keys() {
		if v, ok := p.Get(k); ok {
			...
			fmt.Printf("Internal consistency error. Key %v present in param.Keys() but failed to Get()!", k)
		}
	}
...
func (h *Expires) String() string {
	return fmt.Sprintf("Expires: %d", ((int)(*h)))
}
...
type AuthorizationHeader struct {
	...
	AuthenticationScheme MaybeString

	// Any parameters present in the header.
	Params Params
}

func (authorization *AuthorizationHeader) String() string {
	var buffer bytes.Buffer
	buffer.WriteString("Authorization:")

	switch s := authorization.AuthenticationScheme.(type) {
	case String:
		buffer.WriteString(fmt.Sprintf("%s ", s.String()))
	}

	if authorization.Params.Length() > 0 {
		buffer.WriteString("")
		buffer.WriteString(authorization.Params.ToString(','))
	}
...
func (h *AuthorizationHeader) Copy() SipHeader {
	return &AuthorizationHeader{h.AuthenticationScheme, h.Params.Copy()}
}
...
func (UserAgent UserAgent) String() string {
	return "User-Agent: " + (string)(UserAgent)
}

func (h *UserAgent) Name() string { return "User-Agent" }

func (h *UserAgent) Copy() SipHeader {
	temp := *h
	return &temp
}
...
func (h *Subject) String() string {
	return "Subject: " + h.SendDeviceID + ":" + h.SendChannel + "," + h.ReceiverID + ":" + h.ReceiverChannel
}

func (h *Subject) Name() string {
	return "Subject"
}

func (h *Subject) Copy() SipHeader {
	return &Subject{h.SendDeviceID, h.SendChannel, h.ReceiverID, h.ReceiverChannel}
}
